...
export class CartRepository implements ICartService {
  ...
  async addProductToCart(userId: number, productId: number): Promise<Cart> {
    ...
    const product = await AppDataSource.getRepository(ProductSchema).findOne({ where: { id: productId } });
    ...
    if (!cart) {
      const newCart = this.cartRepository.create({ userId, products: [product] });
